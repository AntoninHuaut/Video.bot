...
client.on('message', msg => {
    ...
    if (!(msg.channel instanceof Discord.DMChannel) && msg.guild.me.hasPermission("MANAGE_MESSAGES") && msg.deletable)
        msg.delete().catch(o => {});

    let content = msg.content.slice(config.discord.prefix.length);

    if (!content) {
        vidUtils.getAllVideosTXT().then(items => {
            msg.channel.send(`<@${msg.author.id}>` + items, {
                split: true
            }).catch(o => {});
        });
    } else
        vidUtils.getVideos(content).then(items => {
            if (items.length == 0)
